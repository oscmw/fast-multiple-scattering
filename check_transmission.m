% Test script for fast acoustic transmission multiple-scattering simulation
%
...
%
%      1) fast_multiple_scattering_transmission.m which implements the 
%         algorithm in [1];
%
...
f = fast_multiple_scattering_transmission(tp,'PLANEWAVE');
...
% set transmission type to acoustic
p.transmissionAcoustic;

% add the scatterers
for j=1:N
    % scatterers are penetrable with density specified
    s{j} = scatterer(pos(j),1,'DIELECTRIC',refin,rho1/rho0);
    p.addScatterer(s{j});
end
