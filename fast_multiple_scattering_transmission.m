% Fast acoustic transmission multiple-scattering simulation
%
%   ff = fast_multiple_scattering_transmission(tp,'PLANEWAVE') computes the far
...
%
%   ff = fast_multiple_scattering_transmission(tp,'POINTSOURCE') computes 
%   the far field ff for the point-source problem (39) in [1]. The exact 
%   solution is known for this problem, and so the error is computed and 
...
function ff = fast_multiple_scattering_transmission(tp,type)
...
% refin, rho0 and rho1 are set by an external function
[refin,rho0,rho1] = set_multiple_scattering_material_constants();
...
switch type
    ...
    case 'POINTSOURCE'
        ...
        % point source located inside the first scatterer as per the test
        % problem in (39) in [1]
        inc0 = point_source(R,kwave);
        inc1 = plane_wave(pi/4,refin*kwave);
...
slvr = solverNystromPenetrable(kwave,[],geom,refin,rho0,rho1);
...
[x,y,qx,qy,dqx,dqy,ddqx,ddqy,nqx,nqy] = geom.geom(pp);
...
for j=1:N
    ...
    if strcmp(type,'POINTSOURCE')
        ...
        % We use the test problem (39) in [1]
        
        % evaluate the exterior field
        p0 = inc0.evaluate(qz);
        [px0,py0] = inc0.evaluateGradient(qz);

        % evaluate the interior field
        p1 = inc1.evaluate(qz);
        [px1,py1] = inc1.evaluateGradient(qz);
        
        % construct the RHS
        b(:,j) = [rho0*p0-rho1*p1;nqx.*(px1-px0) + nqy.*(py1-py0)];

    else
        % evaluate plane wave and its gradient
        p = inc.evaluate(qz);
        [px,py] = inc.evaluateGradient(qz);
        
        % construct the RHS
        b(:,j) = [-rho0*p;nqx.*px + nqy.*py];
    end
...
if strcmp(type,'POINTSOURCE')
    ...
    % source
    ref = inc0.evaluateFarField(exp(1i*tp));
...
%==================================================================
    ...
    function y = matrix_product(x)
        ...
        for j=1:N
            ...
            p = reg{j}.evaluate(qz);
            [px,py] = reg{j}.gradient(qz);

            % sum the field at the Nystrom points on scatterer j
            y(:,j) = y(:,j) + [rho0*p;-(nqx.*px+nqy.*py)];
